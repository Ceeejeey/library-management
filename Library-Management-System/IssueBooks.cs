    ...
    public partial class IssueBooks : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            if (txtEnrollment.Text!= "")
            {
                ...
                cmd.CommandText = "select sName, contact, email from NewStudent where enroll = '" + eid + "'";
                ...
                if (DS.Tables[0].Rows.Count != 0)
                {
                    ...
                    txtContact.Text = DS.Tables[0].Rows[0][1].ToString();
       ...
       private void btnIssueBook_Click(object sender, EventArgs e)
{
    if(txtName.Text != "")
    {
        if(comboBoxBooks.SelectedIndex != -1 && count <= 2)
        {
            ...
            String bookIssueDate = dateTimePicker.Value.ToString("dd MMMM yyyy"); // Correct date format
            ...
            cmd.Connection = con;
            con.Open();
            cmd.CommandText = "insert into IRBook (std_enroll, std_name, std_contact, std_email, book_name, book_issue_date) " +
                              "values ('" + enroll + "', '" + sname + "', '" + contact + "', '" + email + "', '" + bookname + "', '" + bookIssueDate + "')";
            cmd.ExecuteNonQuery();
            con.Close();
